  ...
  return (
    <div className="border border-black/10 rounded-sm hover:shadow-md  flex flex-col justify-center max-w-[400px] bg-white hover:bg-white/90 transition p-3 px-3.5">
      ...
      <div className="flex gap-x-1 items-center">
        ...
        <s className="text-black/70 text-sm">{Math.round(item.price + (item.price * item.discount) / 100)}</s>
        <p className="text-green-800 text-sm font-medium">{item.discount}% off</p>
      </div>
